...
## 7.1 Risk Philosophy: Stewardship over Policing

The Palimpsest Path's approach to risk management is grounded in the same social-psychological framework that underlies the project itself. Rather than treating risk as something to be prevented through restriction — locking away materials, adding warnings, reducing access — the project uses a "Community Stewardship" model in which visible care, thoughtful design, and prosocial priming do the protective work.

This approach draws on Wilson and Kelling's (1982) "Broken Windows" theory, but inverts its logic: rather than rapid removal of disorder signalling that disorder is not tolerated, the regular, visible tending of a cared-for space signals that the space is valued — and valued spaces are treated with greater respect by the communities that use them. The project coordinator's daily presence at the chalk stations, replenishing materials and photographing contributions, is itself a risk mitigation strategy.

This philosophy also shapes the ethics framework. Participants are not subjects to be managed; they are co-authors of a shared space. The consent and privacy architecture is designed to be transparent without being legalistic, and the "implied consent" model is appropriate for a public art intervention of this scale and duration.
...
| Risk | Likelihood | Impact | Mitigation Strategy | Residual |
|------|-----------|--------|---------------------|----------|
| **Chalk dust causing slippery walking surface** | MEDIUM | HIGH | Placement Protocol: drawing zones located exclusively on vertical bannisters. Chalk not applied to horizontal walking planks in high-traffic sections. Rain naturally washes horizontal surfaces clear. Coordinator will sweep or damp-wipe any chalk dust that migrates onto the walkway during the daily audit. This rule should be stated explicitly in the Council permit application and on the QR signage. | LOW |
| **Chalk runoff affecting Huon River / sensitive waterway** | LOW | MEDIUM | Material specification: only ASTM D-4236 certified non-toxic calcium carbonate chalk is used. Chemically inert, poses no documented risk to aquatic ecosystems. No synthetic pigments, solvents, or oil-based materials used at any point. | LOW |
